...
enum BUF_SIZE = 4096;
private void hash_file(FILE *file, ubyte *hash, uint hash_len)
{
  ...
  g_assert(EVP_MD_size(md) == cast(int) hash_len);
...
private uint midi_read_u8(FILE *file)
{
...
private uint midi_read_u16(FILE *file)
{
  guint16 tmp;
  int result;
...
private void vosfile_read_midi(VosSong *song, FILE *file, uint mid_ofs, uint mid_len, double tempo_factor)
{
  ...
  for (i = 0; i < song.ntrack; i++) {
    ...
    g_assert(is_eot);
    g_assert(cast(uint) ftell(file) == track_ofs + track_len);
    track.nevent = event_arr.len;
    track.events = cast(MidiEvent *) g_array_free(event_arr, FALSE);
  }
  g_array_sort(tempo_arr, tempo_change_compare);
  song.ntempo = tempo_arr.len;
  song.tempos = cast(TempoChange *) g_array_free(tempo_arr, FALSE);
  ...
  if (verbose) {
    for (i = 0; i < song.ntempo; i++) {
      ...
      int rtt = cast(int) tc.rt0;
      g_message("Tempo at %d:%02d: %d bpm", rtt / 60, rtt % 60, cast(int) floor(60.0 / tc.qn_time + 0.5));
    }
  }
}

private uint vosfile_read_u8(FILE *file)
{
  int result = getc(file);
...
/* Little endian */
private uint vosfile_read_u16(FILE *file)
{
  guint16 tmp;
  int result;
...
private char *vosfile_read_string(FILE *file)
{
  ...
  result = fread(buf, 1, len, file); g_assert(result == cast(int) len);
  return buf;
}
...
private void vosfile_read_info(VosSong *song, FILE *file, uint inf_ofs, uint inf_len)
{
  ...
  while (1) {
    ...
    cur_ofs = next_ofs; g_assert(cast(uint) ftell(file) == cur_ofs);
  }
  song.num_note_array = note_arrays_arr.len;
  song.note_arrays = cast(NoteArray *) g_array_free(note_arrays_arr, FALSE);
  song.num_user_note = user_notes_arr.len;
  song.user_notes = cast(UserNote *) g_array_free(user_notes_arr, FALSE);
}
